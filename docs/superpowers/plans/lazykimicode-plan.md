...
>
> **Verification:** `pnpm run lint && pnpm run typecheck && pnpm test && pnpm run build` passes (38 test files, 211 tests).
...
**Files:**
- Create: `docs/superpowers/plans/lazykimicode-plan.md`

**Interfaces:**
- Produces: project-local copy of the approved design & implementation plan

- [x] **Step 1: Copy approved plan into project docs**

After plan approval, copy the session plan file to:

```bash
cp /Users/ronan/.kimi-code/sessions/wd_lazykimicode_644da876b5c7/session_2ab245d8-b211-46f8-81b4-ada0ba03c2c1/agents/main/plans/namor-pantha-white-tiger.md /Users/ronan/Documents/projects/lazykimicode/docs/superpowers/plans/lazykimicode-plan.md
```

- [x] **Step 2: Commit**

```bash
git add docs/superpowers/plans/lazykimicode-plan.md
...
- [x] **Step 1: Write `src/shared/types.ts`**
...
- [x] **Step 2: Write `src/shared/paths.ts`**
...
- [x] **Step 3: Write `src/shared/serialize.ts`**
...
- [x] **Step 4: Write unit tests for paths**
...
- [x] **Step 1: Write `src/install/hook-defs.ts`**
...
- [x] **Step 2: Write `src/install/config-patcher.ts`**
...
- [x] **Step 1: Write `src/components/bootstrap/session-start.ts`**
...
- [x] **Step 2: Write `src/components/bootstrap/cli.ts`**
...
- [x] **Step 3: Write `src/components/bootstrap/hooks.json`**
...
- [x] **Step 1: Implement rule discovery**
...
- [x] **Step 1: Implement diagnostics client**
...
- [x] **Step 2: Implement MCP server skeleton**
...
- [x] **Step 1: Implement keyword detector**
...
- [x] **Step 1: Implement steering parser**
...
- [x] **Step 1: Implement daily-active guard**
...
```typescript
import { shouldEmit, getDistinctId, captureDailyActive } from './posthog.js';
import { writeHookOutput } from '../../shared/serialize.js';
...
- [x] **Step 1: Implement recommendation hook**
...
- [x] **Step 1: Implement Boulder check**
...
- [x] **Step 2: Write `team.mjs` state script**
...
// Subcommands add-member, status, archive, delete, etc. are implemented in the full script.
```
...
- [x] **Step 1: Write hook sync script**
...
- [x] **Step 2: Write `plugin/kimi.plugin.json`**
...
- [x] **Step 3: Write `plugin/.mcp.json` (user/project guidance)**
...
- [x] **Step 1: Write `src/install/install-kimi.ts`**
...
- [x] **Step 3: Write `bin/lazykimicode.mjs`**
...
- [x] **Step 4: Add build for installer**
...
- [x] **Step 1: Write `scripts/build.mjs`**
...
- [x] **Step 3: Write release workflow**
...
- [x] **Spec coverage:** Every LazyCodex component maps to a task.
- [x] **Placeholder scan:** No `TODO`/`TBD` remain in task steps (only in component TODOs that are intentionally deferred to implementation).
- [x] **Type consistency:** `HookPayload`, `HookOutput`, `HookDef` used consistently across tasks.
- [x] **Kimi constraints respected:** Hooks go through `config.toml`; plugin manifest does not claim hooks.
- [x] **Validation coverage:** Unit + integration + manual checklist defined.
