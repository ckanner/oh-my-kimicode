...
This skill is designed for the Kimi Code CLI plugin harness. Map its workflow and subagent needs to Kimi tools as follows:

| LazyCodex / Codex concept | Kimi Code equivalent |
|---|---|
| Spawn a focused coding subagent | `Agent(prompt=..., subagent_type="coder")` |
| Explore or plan subagent | `Agent(prompt=..., subagent_type="explore")` or `Agent(prompt=..., subagent_type="plan")` |
| Parallel subagent work (e.g., multi-file review, batch refactoring) | `AgentSwarm` with a prompt template containing `programming` |
| Sequential threaded discussion | sequential `Agent(...)` calls; Kimi has no persistent thread primitive |
| Apply edits / patches | `Write` for new files, `Edit` for incremental changes |
| Browser / web research | Kimi Code CLI has no built-in browser tool. Use the `kimi-webbridge` skill if available, or fall back to `FetchURL`. If neither can reach the target, ask the user. |
| Thread titles | not supported in Kimi; omit |
| Per-skill agent YAML/TOML files | not supported in Kimi; do not create `agents/*.yaml` |

### Evidence rules for this skill

- After every test run, capture and report the tool output verbatim: `EVIDENCE_RECORDED: <test-output>`.
- After every lint/type-check run, capture and report: `EVIDENCE_RECORDED: <lint-output>`.
- After the mandatory post-write review loop, state the pure LOC count and the answers to the 11 self-review questions.
- If a measurement or quality gate fails, stop and fix before declaring the task done.
