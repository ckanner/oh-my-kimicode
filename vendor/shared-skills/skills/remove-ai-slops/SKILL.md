...
description: "Remove AI-generated code smells (slop) from branch changes or an explicit file list. Locks behavior with regression tests FIRST, then runs categorized cleanup via parallel coder subagents in batches of 5, then verifies with quality gates. Covers 10 slop categories including performance equivalences, excessive complexity (object annotations, if/elif variant chains), and oversized modules (250+ pure LOC with mandatory modular refactoring). MUST USE when the user asks to \"remove slop\", \"clean AI code\", \"deslop\", \"clean up AI-generated code\", \"remove AI slop\", or wants to clean up AI-generated patterns from recent changes."
type: prompt
whenToUse: "When the codebase contains low-quality AI-generated artifacts — placeholder comments, dead code, redundant wrappers, unnecessary verbosity, over-defensive checks, duplication, or performance equivalences — especially in a branch diff or explicit file list."
---
...
- **Optional scope**: explicit file list passed by the caller (e.g., a CI workflow's changed-files set)
...
The agent looks for these ten categories. The first three are stylistic, the next three are structural, the next two are about hidden cost, the next is about behavior coverage, and the last is structural/architectural.
...
10. **Oversized modules** — any source file exceeding **250 pure LOC** (non-blank, non-comment lines). This is an architectural defect, not a style preference. Measure: `awk '!/^[[:space:]]*$/ && !/^[[:space:]]*(#|\/\/)/' <file> | wc -l`.
   ...
   **When found, do NOT just flag it. Execute a full modular refactoring:**
   ...
   6. Verify: run the size checker / `awk` measurement again — every file must be ≤250 pure LOC. Run tests, typecheck, lint.
...
| Typecheck | project's type-checker (`tsc`, `mypy`, `pyright`, etc.) on changed files | zero new errors |
...
### Phase 4: Parallel slop removal via coder subagents in batches of 5

Files are processed by `coder` subagents with the `remove-ai-slops` skill loaded, **batched 5 at a time in parallel**.

**Batching protocol** (strict):

1. Slice the in-scope file list into chunks of up to 5 files.
2. For each chunk, launch all `Agent` calls **in a single message** via `AgentSwarm`, using a prompt template that loads the `remove-ai-slops` skill.
3. Wait for the `AgentSwarm` to return results for all agents in the batch.
4. Launch the next batch of 5. Repeat until every file is processed.
...
Return `EVIDENCE_RECORDED:` with a summary of changes, skipped items, and the final test/lint/typecheck status for this file.
"""
)
```

**Batch failure handling**: a subagent that returns without a deliverable, is blocked, or reports a failure should not stop the rest of the batch. Collect successful results and retry the failed file once later. If retry also fails, escalate that file under "Issues Found & Fixed" in the final report.
...
Quality Gates:
  ...
  - Lint: PASS
  - Typecheck (project type-checker): PASS (0 new errors on changed files)
...
- If correctness depends on further inspection, keep using the project's type-checker, the test runner, and direct file reads until the result is grounded.
...
- Use `Agent(subagent_type="explore")` to scan the codebase for slop markers, test coverage gaps, and oversized modules.
- Use `Agent(subagent_type="coder")` for per-file slop removal in batches of up to 5 via `AgentSwarm`.
- Use `Agent(subagent_type="plan")` to review the cleanup plan for oversized-module splits.
- Use `Edit` / `Write` for all file mutations; never use Codex-style `apply_patch`.
- Use the project's own test runner, linter, and type-checker for quality gates. Kimi Code CLI has no built-in `lsp_diagnostics` or browser tool.
- For any work requiring a browser (e.g., verifying UI behavior), use the `kimi-webbridge` skill if available, or `FetchURL` for static pages, or ask the user.
- For parallel file processing, use `AgentSwarm` with a prompt template containing `remove-ai-slops`.
- `Agent` runs to completion; there is no `background_output(task_id=...)` equivalent. Collect results from the `AgentSwarm` response.
- Output `EVIDENCE_RECORDED: <lint-and-test-output>` after each verification step.
