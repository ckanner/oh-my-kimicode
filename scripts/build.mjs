...
const versionFileTs = path.join(__dirname, '..', 'src', 'shared', 'version.ts');
const versionFileMjs = path.join(__dirname, '..', 'src', 'shared', 'version.mjs');
const versionFileContent = `// Generated by scripts/build.mjs; do not edit manually.\nexport const VERSION = '${pkg.version}';\n`;
fs.writeFileSync(versionFileTs, versionFileContent, 'utf-8');
fs.writeFileSync(versionFileMjs, versionFileContent, 'utf-8');
...
async function main() {
  ...
  const pluginManifestPath = path.join(__dirname, '..', 'plugin', 'kimi.plugin.json');
  const manifestRaw = fs.readFileSync(pluginManifestPath, 'utf-8');
  const updatedManifest = manifestRaw.replace(/("version"\s*:\s*")[^"]*(")/, `$1${pkg.version}$2`);
  fs.writeFileSync(pluginManifestPath, updatedManifest, 'utf-8');
