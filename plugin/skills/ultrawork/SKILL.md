...
exercises the surface; capture the artifact.
  ...
  3. Browser use — in Kimi Code CLI, use the `kimi-webbridge` skill
     ...
     Otherwise use the project's configured browser
     tooling (Playwright, agent-browser, dev-browser skill) to drive the
     REAL page; if none is available, ask the user how to capture it.
     Capture action log + screenshot path. Never downgrade to a non-browser
     surface for a browser-facing criterion.
...
`send-keys ...`, the browser action, the `page.click(...)`, the
...
- Use `Agent(prompt=..., subagent_type="coder"|"explore"|"plan")` for delegated
  work. Every `Agent` message starts with `TASK: <imperative assignment>`, then
  names `DELIVERABLE`, `SCOPE`, and `VERIFY`. It runs to completion in one turn;
  there is no `wait_agent`, `background_output(task_id=...)`, or thread polling.
- Use `AgentSwarm` for parallel subtasks. The swarm prompt template contains
  `{{item}}`; each item becomes one subagent invocation. All results return
  together, so dependent work waits for the full swarm output.
- Use `Write` / `Edit` for all file changes. There is no `apply_patch` or thread
  title API; drop any `codex_app.set_thread_title` step.
- Use `TodoList` as the live checklist. Mark one item `in_progress` at a time,
  update on every transition, and mark completed immediately.
- Use `CreateGoal` (or open a `# Goal` block) to set the objective. Do not
  invent a numeric budget or `status` field.
- For unauthenticated browser/page QA, prefer the `kimi-webbridge` skill or the
  project's configured browser tooling; ask the user if neither is available.
- For team orchestration, use the `teammode` skill and `AgentSwarm`; Kimi has
  no persistent subagent threads or `codex_app.*` thread tools.
...
Call `CreateGoal` (or open your reply with a `# Goal` block treated as
...
artifact path the moment it happens. Update `## Now` and `## Todo` on
every transition. Append-only — never rewrite. This notepad
...
`## Now`. Recover state from the notepad; do not re-plan from scratch
or re-run completed steps.

## 3. Register obsessive todos via `TodoList`
The live checklist is `TodoList`. Translate every action from the plan
into one `TodoList` step — one step per atomic work unit: an edit plus
its verification, a QA scenario run, a teardown. Keep each step small
enough to finish within a few tool calls.
Call `TodoList` on EVERY state transition — the instant a step starts
...
- CodeGraph, when `codegraph_*` tools or the `codegraph` MCP exist ->
  use `codegraph_explore` first for how/where/what/flow questions and
  before edits; if absent, inactive/uninitialized, or cold-start
  unavailable, keep moving with Read/Grep/Glob/LSP and the ast-grep skill.
...
- Symbols — definitions, references, rename impact, diagnostics ->
  the `lsp` MCP (or `lsp_goto_definition`, `lsp_find_references`,
  `lsp_symbols`, `lsp_diagnostics` if available). Use the LSP, not text
  search, for anything symbol-shaped.
...
as focused `Agent` calls and keep doing root work while they run.
...
Every `Agent` message is self-contained and starts with
`TASK: <imperative assignment>`, then names `DELIVERABLE`, `SCOPE`, and
`VERIFY`. State that it is an executable assignment, not a context
handoff. Paste only the context the child needs.

`Agent` runs to completion and returns its final message in the same
turn; there is no `wait_agent`, mailbox polling, or `close_agent`. For
parallel work, use `AgentSwarm`. For long-running work, prefer several
smaller focused `Agent` calls over one enormous call. If a child returns
without the required deliverable, respawn a smaller, more focused `Agent`
with the missing piece.

Kimi Code CLI supports only the built-in subagent types
`coder` / `explore` / `plan`. Treat any request for a custom role
(reviewer, planner, explorer, librarian, etc.) as **routing-unverified**:
say so briefly in the notepad, paste the role requirements into the
`prompt`, and judge the result from delivered evidence. Never claim the
reviewer, planner, or explorer role was selected from a TOML profile
unless runtime evidence confirms it.

Treat child status as a progress signal, not a timeout counter. For
work likely to exceed one turn, break it into smaller `Agent` calls or
tell the child to include a concise `WORKING: <task> - <current phase>`
section in its final message, and `BLOCKED: <reason>` only when it cannot
progress. Track spawned agent names locally. Fallback only when the
child is completed without the deliverable, ack-only, or no longer
running. If that followup is still silent or ack-only, record the result
as inconclusive, do not count it as approval/pass, and respawn a smaller
focused `Agent` with the missing deliverable.

# Subagent-dependent transition barrier
Do not mark a `TodoList` step `completed` while an active child owns
...
Spawn every independent child for the current wave first. For parallel
lanes, use `AgentSwarm`; for sequential lanes, call `Agent` one at a time.
After the wave returns, integrate every result before any dependent
`TodoList` transition, `CreateGoal` continuation, implementation tool
call, plan drafting, approval-gate work, PR handoff, or final response.
A missing or incomplete result is not terminal status.
Do not write the final answer, PR handoff, or completion summary while
active subagent work remains open. If a parallel lane is silent or
ack-only after followup, close the lane as inconclusive, record that it
is not approval, and respawn smaller only if the deliverable is still
required.
...
- one wave's independent todos -> one `Agent` each (or an `AgentSwarm`
  for the whole wave); the todo's scope/files become that agent's
  `focus`, and its acceptance criteria + QA become the agent's
  `deliverable`.
- the plan's dependency matrix sets the shape: todos with no unmet
  dependency inside a wave run concurrently via `AgentSwarm`; a todo
  that depends on another waits, so launch the next wave only after the
  blocking agents report.
- todos in the same wave that touch overlapping files -> give those
  agents separate git worktrees (see the `teammode` skill). To land the
  work, merge each member branch with a merge commit, then remove the
  worktrees and archive the team state.

Keep the plan file as the shared spec: point each agent at its todo by
path, and verify the agent's result against that todo's acceptance
criteria before you integrate.
...
1. Spawn a child with `Agent(subagent_type="plan")` and a self-contained
   reviewer assignment in `prompt`. Kimi cannot select a custom reviewer
   role from a TOML profile, so paste the reviewer requirements into the
   message.
